...
namespace LudoEngine.GameLogic.GamePlayers
{
    public class Stephan : IGamePlayer
    {
        ...
        public void Play(IDice dice)
        {
            ...
            if (result.takeout)
            {
                if (result.takeoutCount == 2)
                {
                    ...
                    WriteLogging(LoggerMessage);
                    var basePawns = BoardPawnFinder.PawnsInBase(Board.BoardSquares, Color);
                    basePawns[0].Move(1);
                    basePawns[0].Move(1);
                ...
                else if (result.takeoutCount == 1)
                {
                    ...
                    LoggerMessage += $"\n{DateTime.Now.ToShortTimeString()}: [Method: Play] Calculations finished. Committing move";
                    WriteLogging(LoggerMessage);
                    BoardPawnFinder.PawnsInBase(Board.BoardSquares, Color)[0].Move(diceRoll);
                }
            }
            else
            {
        ...
        {
            var Pawns = BoardPawnFinder.OutOfBasePawns(Board.BoardSquares, Color);
        ...
        private Pawn ReturnFarthestPawn()
        {
            ...
            Pawn pawn = null;
            foreach (var square in BoardNavigation.TeamPath(Board.BoardSquares,Color).Where(square => square.Pawns.Count > 0 &&
        ...
        private (bool result, List<Pawn> pawnsToNotMove) CheckIfPawnWillEndUpInEnemySpawn(int dice)
        {
            ...
            var squarePosition = BoardNavigation.StartSquare(Board.BoardSquares, Color); //Start at spawn
        ...
        private Pawn CheckIfPawnCanReachSafezone(IEnumerable<Pawn> piecesOut, int dice)
        {
            ...
            foreach (var piece in piecesOut)
            {
                ...
                {
                    squarePositionCalc = BoardNavigation.GetNext(Board.BoardSquares, squarePositionCalc, Color);
        ...
        private Pawn CheckIfPawnIsBlockingSpawn()
        {
            ...
            if (BoardNavigation.BaseSquare(Board.BoardSquares, Color).Pawns.Count <= 0)
            {
                LoggerMessage +=
            ...
            var pawnInTakeOut = BoardNavigation.StartSquare(Board.BoardSquares, Color).Pawns.Find(pawn => pawn.Color == Color);
        ...
        private Pawn CheckIfPawnDistanceIsTooGreat()
        {
            ...
            var squarePosition = BoardNavigation.StartSquare(Board.BoardSquares, Color); //Start at spawn
            var furthestIndex = 0;
            for (var i = 0; i <= BoardNavigation.TeamPath(Board.BoardSquares, Color).Count; i++)
            {
                squarePosition = BoardNavigation.GetNext(Board.BoardSquares, squarePosition, Color);
            ...
            squarePosition = BoardNavigation.StartSquare(Board.BoardSquares, Color); //Start at spawn
