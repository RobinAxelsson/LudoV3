...
namespace LudoEngine.GameLogic.GamePlayers
{
    public class Stephan : IGamePlayer
    {
        ...
        public void Play(IDice dice)
        {
            ...
            if (result.takeout)
            {
                if (result.takeoutCount == 2)
                {
                    ...
                    WriteLogging(LoggerMessage);
                    var basePawns = BoardPawnFinder.PawnsInBase(StaticBoard.BoardSquares, Color);
                    basePawns[0].Move(1);
                    basePawns[0].Move(1);
                ...
                else if (result.takeoutCount == 1)
                {
                    ...
                    LoggerMessage += $"\n{DateTime.Now.ToShortTimeString()}: [Method: Play] Calculations finished. Committing move";
                    WriteLogging(LoggerMessage);
                    BoardPawnFinder.PawnsInBase(StaticBoard.BoardSquares, Color)[0].Move(diceRoll);
                }
            }
            else
            {
        ...
        {
            var Pawns = BoardPawnFinder.OutOfBasePawns(StaticBoard.BoardSquares, Color);
        ...
        private (bool result, List<Pawn> pawnsToNotMove) CheckIfPawnWillEndUpInEnemySpawn(int dice)
        {
            ...
            var squarePosition = BoardNavigation.StartSquare(StaticBoard.BoardSquares, Color); //Start at spawn
        ...
        private Pawn CheckIfPawnCanReachSafezone(IEnumerable<Pawn> piecesOut, int dice)
        {
            ...
            {
                var squarePosition = StaticBoard.BoardSquares.Find(square => square == piece.CurrentSquare());
        ...
        private Pawn CheckIfPawnIsBlockingSpawn()
        {
            ...
            if (BoardNavigation.BaseSquare(StaticBoard.BoardSquares, Color).Pawns.Count <= 0)
            {
                LoggerMessage +=
            ...
            var pawnInTakeOut = BoardNavigation.StartSquare(StaticBoard.BoardSquares, Color).Pawns.Find(pawn => pawn.Color == Color);
        ...
        private Pawn CheckIfPawnDistanceIsTooGreat()
        {
            ...
            var squarePosition = BoardNavigation.StartSquare(StaticBoard.BoardSquares, Color); //Start at spawn
            var furthestIndex = 0;
            for (var i = 0; i <= BoardNavigation.TeamPath(StaticBoard.BoardSquares, Color).Count; i++)
            {
                squarePosition = BoardNavigation.GetNext(StaticBoard.BoardSquares, squarePosition, Color);
            ...
            squarePosition = BoardNavigation.StartSquare(StaticBoard.BoardSquares, Color); //Start at spawn
