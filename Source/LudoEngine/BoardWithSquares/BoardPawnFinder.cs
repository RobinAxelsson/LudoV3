...
namespace LudoEngine.BoardUnits.Main
{
    public static class BoardPawnFinder
    {
        public static List<Pawn> PawnsInBase(List<IGameSquare> boardSquares, TeamColor color) =>
            boardSquares.FirstOrDefault(x => x.GetType() == typeof(BaseSquare) && x.Color == color)?.Pawns ?? Array.Empty<Pawn>().ToList();
        ...
        public static List<Pawn> AllPlayingPawns(List<IGameSquare> boardSquares) => boardSquares.FindAll(x => x.GetType() != typeof(BaseSquare) && x.GetType() != typeof(GoalSquare)).SelectMany(x => x.Pawns).ToList();
